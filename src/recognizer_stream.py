...
while True:
    ...
    if frames%3 == 0:
        ...
        if len(bboxes) != 0:
            ...
            for bboxe in bboxes:
                ...
                # Compare this vector to source class vectors to verify it is actual belong to this class
                    ...
                    # print("Recognized: {} <{:.2f}> ".format(name, cos_similarity))
                    if cos_similarity < cosine_threshold and proba > proba_threshold:
                        name = le.classes_[j]
                        text = "{}".format(name)
                        print("Recognized: {} <{:.2f}>".format(name, proba*100))
                        print("cos_sinSim: {} <{:.2f}> ".format(name, cos_similarity))
                # Start tracking
                ...
                cv2.putText(frame, text, (bbox[0], y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        ...
        for tracker, text in zip(trackers,texts):
            ...
            cv2.putText(frame, text, (startX, startY - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255),1)
